...
1. 首页输入一个 B 站视频 URL，展示资产创建或复用。
2. 进入资产页，依次抽帧、视觉分析、抓字幕/ASR、构建资产。
3. 展示关键帧的时间戳、画面文字、信息密度和视觉事实。
4. 进入生成页，用同一资产生成“图文总结”和“证据卡片”。
5. 多选两个或三个 ready 资产，生成“多视频综合”。
6. 指出生成页的复用证明：没有重新处理视频，只复用了已保存资产层。
